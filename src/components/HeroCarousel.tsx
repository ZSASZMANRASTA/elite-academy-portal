...
const HeroCarousel = () => {
  const { data: slides } = useSiteContent<HeroSlide[]>("hero", []);
  const current = (slides ?? []).filter((s) => s && s.src);
  ...
  if (current.length === 0) {
    return <div className="relative h-full w-full overflow-hidden bg-muted" />;
  }
